...
public:
	...
	{
		if (!typeId)
			throw std::runtime_error(encrypt("Invalid type id to register"));
	...
	{
		if (!typeId)
			throw std::runtime_error(encrypt("Invalid type id to resolve"));
	...
	std::shared_ptr<T> Resolve(const std::string& id)
	{
		...
		throw std::runtime_error(encrypt("Could not locate type in IoC"));
	}
